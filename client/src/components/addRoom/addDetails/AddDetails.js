...
const AddDetails = () => {
  ...
  const handleCostTypeChange = (e) => {
    ...
    if (costType === 0) {
      ...
      dispatch({ type: 'UPDATE_DETAILS', payload: { price: 1500 } });
    }
  };
    ...
    >
      <FormControl>
        ...
        >
          ...
          <FormControlLabel value={1} control={<Radio />} label={<><div>Nominal Fee</div><div>(per hour)</div></>} />
          {Boolean(costType) && (
            <TextField
              ...
              inputProps={{ type: 'number', min: 100, max: 10000, step: 100 }}
              value={price}
